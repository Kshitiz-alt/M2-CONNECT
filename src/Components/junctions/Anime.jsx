  ...
  return (
    <div className="pages">
      <nav id="back" className="flex items-center gap-[20em]">
        ...
        <form className="flex w-[30%] h-[30px] justify-self-center items-center bg-white p-4 rounded-[20px]" onSubmit={handleSearch} role="search">
          <input
            className="border-none outline-none rounded-full h-[30px] w-full"
            type="search"
            placeholder="Search..."
            aria-label="Search"
      ...
      <div className="flex gap-[5em] overflow-x-scroll">
        {albums?.map((album) => (
          <div key={album.id} className="flex flex-col justify-center items-center text-white gap-[3em] px-10 h-[90vh]">
